...
**The reviewer is told what has already been decided here.** A proposal that has been reviewed before carries the findings earlier runs left on it, and the question now names them: which were waived, by whom, and with what reason, and which are still open. The instruction attached is to not re-file a waived one — not reworded, not under another category, not at a higher severity. This is the cause-side half of a problem the [screening](../screening-the-answer/README.md) capability can only treat afterwards: a waiver is recognised by a finding's title, and the title is prose the model rewrites every run, so a rejected claim could come back indefinitely under new wordings. Asking it not to is the only remedy that also covers the case of the same claim arriving under a different category. The list is capped like every other retrieved block, with waived findings listed first, because re-filing a waived finding costs a reviewer an argument while re-filing an open one costs nothing.

**One brief, many batches.** The instruction block is deliberately identical across every batch in a run, so a provider that supports it can charge for the shared preamble once and read it at cache rates thereafter. Anything that varies per batch lives in the question instead — including the record of what was already decided, which is the same for every batch but belongs on the question side, since it is a fact about this proposal rather than about the job.
